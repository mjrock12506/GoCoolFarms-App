...
export default function OrdersScreen() {
  ...
  const [activeTab, setActiveTab] = useState(route.params?.initialFilter ?? 'All')
  ...
  useEffect(() => {
    ...
    if (newFilter && newFilter !== activeTab) setActiveTab(newFilter)
  ...
  useEffect(() => {
    applyAllFilters(activeTab, orders, searchName, itemFilter, dateFrom, dateTo)
  }, [activeTab, orders, searchName, itemFilter, dateFrom, dateTo])
  ...
  function applyAllFilters(
    ...
    switch (tab) {
      ...
      case 'Confirmed': result = result.filter(o => o.status === 'confirmed'); break
      case 'Assigned': result = result.filter(o =>
        o.status === 'assigned' || o.status === 'out_for_delivery'); break
      case 'Delivered': result = result.filter(o => o.status === 'delivered'); break
      case 'Paid': result = result.filter(o => PAID_STATUSES.includes(o.payment_status)); break
      case 'Unpaid': result = result.filter(o =>
        !PAID_STATUSES.includes(o.payment_status) && o.status !== 'cancelled'); break
    ...
    if (name.trim()) {
      ...
      result = result.filter(o =>
        o.customer?.full_name?.toLowerCase().includes(lower))
    }
    ...
    if (item !== 'All Items') {
      result = result.filter(o =>
        o.order_items?.some(i =>
          i.product_name.toLowerCase().includes(item.toLowerCase().replace(' lbs', '').trim())
        )
      )
    }

    // Date from filter (ordered_at >= dateFrom)
    if (from) {
      const fromStart = new Date(from)
      fromStart.setHours(0, 0, 0, 0)
      result = result.filter(o => {
        if (!o.ordered_at) return false
        return new Date(o.ordered_at) >= fromStart
      })
    }

    // Date to filter (ordered_at <= dateTo)
    if (to) {
      const toEnd = new Date(to)
      toEnd.setHours(23, 59, 59, 999)
      result = result.filter(o => {
        if (!o.ordered_at) return false
        return new Date(o.ordered_at) <= toEnd
      })
    }
  ...
  function tabCount(tab: string, allOrders: Order[]) {
    switch (tab) {
      ...
      case 'Paid': return allOrders.filter(o => PAID_STATUSES.includes(o.payment_status)).length
  ...
  async function loadOrders() {
    try {
      const { data, error } = await supabase
        .from('orders')
        .select(`
          id, status, delivery_date, ordered_at,
  ...
  async function cancelOrder(orderId: string) {
    Alert.alert('Cancel Order?', 'This will cancel the order. You can undo this.', [
  ...
  const hasActiveFilters = searchName || itemFilter !== 'All Items' || dateFrom || dateTo
        ...
        <View style={styles.filterPanel}>
          ...
          >
            {ITEM_FILTERS.map(f => (
              <TouchableOpacity
                key={f}
                style={[styles.chip, itemFilter === f && styles.chipActive]}
                onPress={() => setItemFilter(f)}
              >
                <Text style={[styles.chipText, itemFilter === f && styles.chipTextActive]}>
                  {f}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          {/* Date range */}
          <Text style={styles.filterLabel}>Filter by order date</Text>
          <View style={styles.dateRow}>
            <TouchableOpacity
              style={styles.dateBtn}
              onPress={() => setShowFromPick(true)}
            >
              <Text style={styles.dateBtnText}>
                {dateFrom ? `From: ${fmtDate(dateFrom)}` : 'From date'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.dateBtn}
              onPress={() => setShowToPick(true)}
            >
              <Text style={styles.dateBtnText}>
                {dateTo ? `To: ${fmtDate(dateTo)}` : 'To date'}
              </Text>
            </TouchableOpacity>
          </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            <Text style={styles.emptyIcon}>
              {hasActiveFilters ? '🔍' : activeTab === 'Cancelled' ? '🚫' : activeTab === 'Paid' ? '💰' : '📋'}
            </Text>
            <Text style={styles.emptyText}>
              {hasActiveFilters ? 'No orders match your filters' : `No ${activeTab.toLowerCase()} orders`}
            </Text>
            {hasActiveFilters && (
            ...
            <View style={styles.cardBody}>
              ...
              <Text style={styles.items} numberOfLines={1}>
                🛒 {item.order_items?.length > 0
                  ? item.order_items.map(i => `${i.product_name} x${i.quantity}`).join('  ·  ')
                  : 'No items'}
              </Text>
              ...
              {item.ordered_at && (
                <Text style={styles.orderedAt}>
                  🕐 {new Date(item.ordered_at).toLocaleDateString('en-US', {
                ...
                <View style={styles.bottomLeft}>
                  ...
                  <View style={[styles.badge, {
                    ...
                    <Text style={styles.badgeText}>
                      {item.payment_method?.toUpperCase()} · {item.payment_status.replace(/_/g, ' ')}
                    </Text>
                  </View>
                </View>
                <Text style={styles.date}>📅 {item.delivery_date ?? 'No date'}</Text>
              </View>
              ...
              <View style={styles.actionRow}>
                ...
                {item.status === 'cancelled' && (
                  <TouchableOpacity style={styles.undoBtn} onPress={() => undoCancel(item.id)}>
...
const styles = StyleSheet.create({
  ...
  tab: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, backgroundColor: '#F1F5F9' },
  ...
  // Search
  searchRow: { flexDirection: 'row', paddingHorizontal: 12, paddingVertical: 8, gap: 8, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  searchInput: { flex: 1, backgroundColor: '#F1F5F9', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, fontSize: 13, color: '#0F172A' },
  filterToggleBtn: { backgroundColor: '#F1F5F9', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, justifyContent: 'center' },
  filterToggleBtnActive: { backgroundColor: '#8B5CF6' },
  filterToggleText: { fontSize: 12, color: '#64748B', fontWeight: '600' },
  filterToggleTextActive: { color: '#fff' },

  // Filter panel
  filterPanel: { backgroundColor: '#fff', paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#E2E8F0' },
  filterLabel: { fontSize: 11, fontWeight: '700', color: '#94A3B8', marginTop: 10, marginBottom: 6, textTransform: 'uppercase' },
  chipRow: { gap: 6 },
  ...
  dateBtn: { flex: 1, backgroundColor: '#F1F5F9', borderRadius: 10, padding: 10, alignItems: 'center' },
  dateBtnText: { fontSize: 12, color: '#475569', fontWeight: '600' },
  clearBtn: { marginTop: 10, backgroundColor: '#FEF2F2', borderRadius: 8, paddingVertical: 8, alignItems: 'center', borderWidth: 1, borderColor: '#FECACA' },
  clearBtnText: { fontSize: 12, fontWeight: '700', color: '#EF4444' },

  // Revenue
  revenueStrip: { backgroundColor: '#DCFCE7', padding: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  revenueLabel: { fontSize: 13, fontWeight: '700', color: '#2D6A4F' },
  revenueAmount: { fontSize: 20, fontWeight: '900', color: '#2D6A4F' },
  countText: { fontSize: 12, color: '#94A3B8', paddingHorizontal: 16, paddingVertical: 8 },

  // Cards
  card: { backgroundColor: '#fff', borderRadius: 12, marginBottom: 12, flexDirection: 'row', elevation: 2, overflow: 'hidden' },
  ...
  testBadge: { backgroundColor: '#F1F5F9', borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1 },
  testBadgeText: { fontSize: 8, fontWeight: '800', color: '#94A3B8' },
  cancelledBadge: { backgroundColor: '#FEE2E2', borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1 },
  cancelledBadgeText: { fontSize: 8, fontWeight: '800', color: '#EF4444' },
  paidBadge: { backgroundColor: '#DCFCE7', borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1 },
  ...
  items: { fontSize: 12, color: '#475569', marginTop: 4, fontWeight: '500' },
  orderedAt: { fontSize: 11, color: '#94A3B8', marginTop: 2, fontStyle: 'italic' },
  ...
  cancelBtn: { backgroundColor: '#FEF2F2', borderRadius: 8, paddingVertical: 7, alignItems: 'center', borderWidth: 1, borderColor: '#FECACA' },
  cancelBtnText: { fontSize: 12, fontWeight: '700', color: '#EF4444' },
  undoBtn: { backgroundColor: '#EFF6FF', borderRadius: 8, paddingVertical: 7, alignItems: 'center', borderWidth: 1, borderColor: '#BFDBFE' },
